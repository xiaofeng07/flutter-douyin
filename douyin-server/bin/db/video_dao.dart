...
class VideoDao {
  ...
  Future<List<Map<String, dynamic>>> queryVideoList() async {
    var result = await DatabaseManager.pool.execute(''' SELECT
	id, 
	user_id, 
	title, 
	alias, 
...
FROM
  ...
  Future<List<Map<String, dynamic>>> queryFriendsVideoList(friendIds) async {
    var result = await DatabaseManager.pool.execute('''SELECT
	id, 
	user_id, 
	title, 
  ...
  END AS is_collected ,  
	...
	topic
FROM
	tb_video_info WHERE user_id IN($friendIds)''');
